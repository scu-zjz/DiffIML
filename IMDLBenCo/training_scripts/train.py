...
def get_args_parser():
    ...
    parser.add_argument('--save_epochs', type=str, default='',
                        help='Comma-separated list of 1-based epochs to also save checkpoints')
...
def main(args, model_args):
    ...
    if getattr(args, 'save_epochs', ''):
        try:
            save_epochs = set(int(x.strip()) for x in args.save_epochs.split(',') if x.strip())
        except Exception:
            print("Warning: --save_epochs parse error, fallback to default milestones.")
            save_epochs = {21, 37, 45, 61, 85, 105, 125, 149}
    else:
        save_epochs = {21, 37, 45, 61, 85, 105, 125, 149}
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #     pass
            ...
            #     loss_scaler=loss_scaler, epoch=epoch)
        if args.output_dir:
            cur_epoch_1based = epoch + 1
            should_save_regular = (epoch % 50 == 0 and epoch != 0) or (cur_epoch_1based == args.epochs)
            should_save_extra = (cur_epoch_1based in save_epochs)
            if should_save_regular or should_save_extra:
                misc.save_model(
                    args=args,
                    model=model,
